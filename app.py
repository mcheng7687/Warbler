...
def users_show(user_id):
    ...
    return render_template('users/show.html', user=user, messages=messages, num_of_likes=num_of_likes(user_id))
...
def homepage():
    ...
    if g.user:
        messages = (Message
                    .query
                    .filter(Message.user_id.in_([following.id for following in g.user.following] + [g.user.id]))
                    .order_by(Message.timestamp.desc())
                    .limit(100)
                    .all())
        likes = Likes.query.filter_by(user_id = g.user.id).all()
        liked_messages_id = [like.message_id for like in likes]

        return render_template('home.html', messages=messages, liked_messages_id=liked_messages_id)
...
#
def num_of_likes(user_id):
    return Likes.query.filter(Likes.user_id == user_id).count()

def add_or_remove_likes(message_id):
    like = (Likes
            .query
            .filter(Likes.user_id == g.user.id, Likes.message_id == message_id)
            .one_or_none())

    if like:
        db.session.delete(like)
    else:
        user_liked = Likes(user_id = g.user.id, message_id = message_id)
